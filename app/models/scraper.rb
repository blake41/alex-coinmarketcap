class Scraper
  ...
  def run(date = 20130428)
    page = bot.get("#{url}/all/views/all/")
    page.links_with(:href => /currencies\/\w+\/\z/).each do |link|
      get_data_for_currency(link, date, url)
    end
  end
  ...
  def add_new_coins(date = 20130428)
    Currency.last.destroy
    page = bot.get("#{url}/all/views/all/")
    page.links_with(:href => /currencies\/\w+\/\z/).uniq {|link| link.href}.each do |link|
      next if Currency.find_by_ticker(link.text)
      get_data_for_currency(link, date, url)
    end
  end

  def get_data_for_currency(link, start_date, url)
    currency_page = mechanize_request {link.click}
    uri = link.uri
    currency_name = uri.path.match(/\/\w+\/(\w+)/)[1]
  ...
  def get_last_day_run
    Currency.find_by_name(:bitcoin).first.date
  end
